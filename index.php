...
if (file_exists('.env')) {
  ...
  foreach($env_vars as $env_var) {
    ...
    if (!defined($env_var_pieces[0])) {
      define(strtoupper($env_var_pieces[0]), str_replace(PHP_EOL, '', $env_var_pieces[1]));
    }
  }
}
$services = array();
if (defined('SERVICES')) {
  ...
  foreach ($services_pieces as $services_piece) {
    ...
    if (isset($service_pieces[0]) && isset($service_pieces[1])) {
      if (!isset($service_pieces[2])) {
        $service_pieces[2] = '';
      }
      $services[] = array("port" => $service_pieces[1], "service" => $service_pieces[0], "ip" => $service_pieces[2]);
    }
  }
} else {
  $services[] = array("port" => "80", "service" => "HTTP", "ip" => "");
  $services[] = array("port" => "21", "service" => "FTP", "ip" => "");
  $services[] = array("port" => "3306", "service" => "MYSQL", "ip" => "");
  $services[] = array("port" => "22", "service" => "SSH", "ip" => "");
...
foreach ($top_cpu_use as $key => $top_cpu_use_item) {
  $top_cpu_use_item_pieces = explode(' ', $top_cpu_use_item);
  if (is_numeric($top_cpu_use_item_pieces[0])) {
    //possible convert to percentages here later on
  } else {
    $top_cpu_use[$key] = str_replace('COMMAND','PROCESS',$top_cpu_use[$key]);
  }
}
